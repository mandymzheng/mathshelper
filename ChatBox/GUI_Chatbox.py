...
def getResponse(ints, intents_json):
    ...
    for i in list_of_intents:
        if(i['tag'] == tag):
            result = random.choice(i['responses']) + random.choice(i['link'])
            break
    return result 

def getImage(ints, intents_json):
    tag = ints[0]['intent']
    list_of_intents = intents_json['intents']
    for i in list_of_intents:
        if(i['tag'] == tag):
            if (i['image'] != ""):
                image = base64.b64decode(i['image'][0])
                break
    return image
...
def send():
    ...
    if msg != '':
        ...
        image = getImage(ints, intents)
        
        ChatBox.insert(END, "Bot: " + res + image + '\n\n')
...
ChatBox.place(x=6, y=6, height = 416, width = 370)
